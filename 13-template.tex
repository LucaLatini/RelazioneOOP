...
\title{Relazione \\``Geometry-Bash''}

\author{Elena Montalti, Ludovico Riccio, Luca Latini e Paolo De Mori}
\date{\today}
...
\begin{abstract}
Questo documento è la relazione del progetto del corso di "Programmazione a Oggetti" a.a 24/25.
%
Realizzato da Elena Montalti, Ludovico Riccio, Luca Latini e Paolo De Mori.
%
Il gruppo si è prodigato a sviluppare un clone del celebre gioco 2d a scorrimento orizzontale 
%
Geometry Dash, ma con una ispirazione al terminale bash Linux.
%
La relazione esamina le varie fasi di sviluppo del progetto, la suddivisione del lavoro e le tecnologie utilizzate;
%
mantenendo particolare attenzione all'utilizzo delle principali metodologie di sviluppo OO 
%
aderendo il più possibile alle buone tecniche di design e sviluppo analizzate nel corso.
\end{abstract}
...
Il gruppo si pone l'obiettivo di realizzare un videogioco 2D a scorrimento orizzontale ispirato fortemente a "Geometry-Dash",
%
che venne sviluppato nell'agosto del 2013 e da allora ha guadagnato una grande notorietà e fama tra gli amanti della categoria,
%
sia per il suo gameplay scorrevole, sia per la sua natura frenetica e le meccaniche ritmiche.
%
Il giocatore ha come obiettivo quello di completare interamente un livello, schivando gli ostacoli presenti e interagendo con oggetti presenti all'interno del percorso.
%
Ogni contatto del personaggio con un ostacolo, provoca la terminazione della partita corrente e l'inizio di una nuova.
%
Il gruppo vuole creare una variante ispirata al design dei terminali bash, fornendo le meccaniche di base del celebre gioco con una diversa veste grafica, 
%
ma senza perderne le ispirazioni principali.

\section{Descrizione e requisiti}

\subsubsection{Requisiti funzionali}
\begin{itemize}
	\item Sviluppo di un livello base, giocabile, in cui il personaggio principale, scorre da sinistra verso destra nello schermo.
	\item Personaggio principale reattivo all'input dell'utente.
	\item Interazione del personaggio con elementi di gioco: diverse tipologie di ostacoli e elementi interagibili o collezionabili dal personaggio.
	\item Possibilità di personalizzazione del personaggio, tramite appositò menù.
	\item Gestione di una valuta cumulativa, ottenibile durante il game-play, che permette di ottenere nuovi livelli di personalizzazione del personaggio o di continuare la partita dal punto in cui si è interrotta.
\end{itemize}

\subsubsection{Requisiti non funzionali}
\begin{itemize}
	\item Possibilità di giocare a diversi livelli di risoluzione.
	\item Creazione di un gameplay fluido e godibile dall'utente.
	\item Possibilità di giocare un livello salvato su file.
	\item Realizzazione di una interfaccia di interazione che simula un terminale bash, con cui interagire tramite comandi fittizzi.
	\item Implementazione di un software usufruibile su Windows, Mac Os e Linux.
\end{itemize}


\section{Modello del Dominio}

\subsection{Legenda termini specifici e alternativi}
\begin{itemize}
	\item Morte del personaggio: Il personaggio principale ha fallito il suo obiettivo di arrivare alla fine del livello, dovrà ricominciare 
	o dal punto di rinascità di default o, pagando monete di gioco, dal punto in cui è avvenuto l'evento che ha determinato il fallimento del livello.
	\item Protagonista: Personaggio principale.
	\item Personaggio: Personaggio principale.
\end{itemize}

\subsection{Entità }
E' una qualsiasi entità del mondo di gioco, è in grado di fornire la propria posizione. Rappresenta la base comune di tutte le entità presenti nella mappa.

\subsection{Personaggio Principale}
L'entità personaggio è l'unico oggetto controllabile e dinamico, funge da interfaccia diretta tra l'utente e l'ambiente di gioco.
In esso sono definiti attributi per la gestione della fisica e geometria, che ne influenzano l'interazione con gli altri elementi di gioco.
Interagisce con l'input utente, gestendo la dinamica del salto eseguendolo,se possibile, quando richiesto.
Il personaggio può collezionare monete all'interno del livello, interagire con entità che ne modificano lo stato, scontrarsi con ostacoli o arrivare alla fine della mappa determinando la vittoria del livello.
%
\subsection{Ostacoli}
L'entità ostacolo è un oggetto statico, inamovibile.L'insieme di ostacoli e power up compone il mondo di gioco.
Ogni ostacolo fornisce: la sua posizione nel livello, informazioni riguardanti la propria geometria e su come il gioco deve reagire in caso di un contatto con il giocatore. 
Il livello è composto da ostacoli di diversi tipi, ogni ostacolo può comportare ,se urtato in un certo modo, la morte del personaggio principale.
%
\subsubsection{Spike}
Un oggetto di forma triangolare che se colpito in qualunque modo, comporta la morte del protagonista.
%
\subsubsection{Box}
L'oggetto standard, di forma quadrata, su cui il personaggio può salire e muoversi. Se colpito lateralmente provoca la morte del giocatore.
%
\subsection{PowerUp}
L'entità powerup è un oggetto statico, che in caso di contatto con il player può comportare modifiche allo stato del protagonista, alla logica o alla sessione di gioco.
Fornisce informazioni sulla posizione, geometria e logica di collisione con il player.
%
\subsubsection{Moneta}
Una moneta è un power up che in caso di collisione col player viene collezionato come valuta di gioco.
%
\subsection{Livello}
Il livello è l'insieme degli Ostacoli e del personaggio. Contiene informazioni sulla posizione e sulla disposizione spaziale delle varie entità nel mondo di gioco.


\includegraphics[width=\textwidth]{img/analisiUml.png}
